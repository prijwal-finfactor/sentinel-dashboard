...
const mockTriggers: TriggerSummary[] = [
  {
    ...
    severity: "high",
    hitCount: 15,
    description: "Transactions exceeding $10,000 in cash",
  },
  {
    id: "t2",
    ...
    severity: "medium",
    hitCount: 8,
    description: "Multiple transactions within short time frame",
  },
  {
    id: "t3",
    ...
    severity: "critical",
    hitCount: 3,
    description: "Customer matches sanctions database",
  },
  {
    id: "t4",
...
export const api = {
    ...
    try {
      ...
      if (isAuthenticated()) {
        ...
        return tenants.map((t: any, index: number) => ({
          ...
          description:
            t.description || `Tenant for ${t.fiuId || "Financial Institution"}`,
  ...
  async fetchProcessRunDetail(runId: string): Promise<ProcessRunDetail> {
    ...
    const userTriggerSummaries: UserTriggerSummary[] = selectedUsers.map(
      (user) => {
        const userTriggers = [...mockTriggers]
          .sort(() => 0.5 - Math.random())
          .slice(0, Math.floor(Math.random() * 3) + 2);
        const triggerHitCount = userTriggers.reduce(
          (sum, trigger) => sum + trigger.hitCount,
          0
        );
  ...
  async fetchTriggerDetail(
    ...
    const triggerTransactions = [...mockTransactions]
      .sort(() => 0.5 - Math.random())
      .slice(0, trigger.hitCount);
  ...
  async uploadProcess(formData: FormData): Promise<Process> {
    ...
    try {
      const response = await apiClient.post(
        "http://localhost:8080/api/v1/ews/consents/upload",
        formData,
        {
          headers: {
